...
def add_to_collection(collection_name, match_df):
    ...
    file_exists = os.path.isfile(collection_name + '.csv')
    match_df.to_csv(collection_name + '.csv', mode='a',
    ...
    match_df = match_df[[
        ...
        "utility_advantage"]]
    file_exists = os.path.isfile(collection_name + '_short.csv')
    match_df.to_csv(collection_name + '_short.csv', mode='a',
                    header=not file_exists, index=False)
    
    return

def collection_summary(collection_name):
    if os.path.isfile(collection_name + '.csv'):
        collection_df = pd.read_csv(collection_name + '.csv')
        ...
        collection_grouped_df.to_csv(
            collection_name + '_summary.csv', index=False)
    else:
...
def identify_in_summary(collection_name, player_filename):
    if not os.path.isfile(collection_name + '_summary.csv'):
        print("Collection does not exist")
    elif not os.path.isfile(player_filename + '.csv'):
        print("Player file does not exist")
    else:
        collection_df = pd.read_csv(collection_name + '_summary.csv')
        player_identification_df = pd.read_csv(player_filename + '.csv')
        ...
        collection_df.to_csv(
            collection_name + '_summary_identified.csv', index=False)
...
def split_by_role_and_save(collection_name, identified_collection_df, role):
    ...
    role_rankings_df.to_csv('position_' + str(role) + '_' +
        collection_name + '_rankings.csv', index=False)

def rank_identified_collection(collection_name):
    if not os.path.isfile(collection_name + '_summary_identified.csv'):
        print("Identified collection does not exist")
    else:
        identified_collection_df = pd.read_csv(collection_name +
        ...
        for role in range(1, 6):
            split_by_role_and_save(collection_name,
                                   identified_collection_df, role)
